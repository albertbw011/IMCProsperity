...
class Trader:
    ...
    starfruit_prices = pd.DataFrame(columns=['mid_price', 'MACD', 'Signal', 'Histogram'])

    # input: OrderDepth order_depth
    # return: mid_price
    def calculate_mid_price(self, order_depth: OrderDepth):
        bid_price = max(list(order_depth.buy_orders.keys()))
        ask_price = min(list(order_depth.sell_orders.keys()))
    ...
    def calculate_macd(self, data, short_period=12, long_period=26, signal_period=9):
        ...
        short_ema = data['mid_price'].ewm(span=short_period, adjust=False).mean()
        long_ema = data['mid_price'].ewm(span=long_period, adjust=False).mean()
    ...
    def extract_features(self, data: pd.DataFrame, state: TradingState, symbol: str):
        trades = state.order_depths[symbol]
        mid_price = self.calculate_mid_price(trades)
        data.loc[state.timestamp, 'mid_price'] = mid_price
    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        for product in state.order_depths:
            ...
            if product == "STARFRUIT":  
                ...
                self.starfruit_prices = self.extract_features(self.starfruit_prices, state, product)

                if len(self.starfruit_prices) < 26:
                    continue

                last_row = self.starfruit_prices.iloc[-1]
                second_last_row = self.starfruit_prices.iloc[-2]

                # MACD crosses above signal line
                # bullish signal
                if len(order_depth.sell_orders) != 0:
                    if second_last_row['Histogram'] < 0 and last_row['Histogram'] > 0:
